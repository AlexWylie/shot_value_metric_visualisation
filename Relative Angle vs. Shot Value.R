...
DF <- DF[!DF$Penalty == 1,]
png(filename = "Relative Angle vs. Shot Value Plot.png", units = "in", width = 10, height = 8, res = 300)
g <- ggplot(DF, aes(x = relativeangle, y = shotvalue))
g + geom_point(alpha = 2/3, colour = "orange") + geom_smooth(method = "auto") + labs(x = "Relative Angle to Goal", y = "Shot Values", title = "Relationship Between Relative Angle and Shot Values") +
