...
DF <- DF[!DF$Penalty == 1,]
DF$shotvalue <- log(DF$shotvalue)
png(filename="Shot Value Map.png", units="in", width=10, height=8, res=300)
ggplot(DF, aes(x = X, y = Y)) + pitch_markings(col_pitch = "white", col_lines = "black") +
        theme_pitch() + xlim(0, 100) + ylim(0, 100) + theme(legend.position="none", plot.title = element_text(hjust = 0.5)) +
        geom_point(aes(colour = shotvalue), size = 3, alpha = 2/3) + scale_color_gradient(low="green", high="red") + 
        xlim(head(DF[order(DF$X),"X"], 1) - 5, 100) + coord_flip()
dev.off()