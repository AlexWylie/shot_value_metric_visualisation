...
R2$shotValueRatio[is.na(R2$R2$shotValueRatio)] <- 0
...
R2$goalRatioPerGameSample1 <- 0
R2$goalRatioPerGameSample2 <- 0
R2$goalRatioPerGameR2 <- 0
for(i in 1:nrow(R2)){
        R2$goalRatioPerGameSample1[i] <- mean(R2$goalRatio[1:i])
}
for(i in nrow(R2):1){
        R2$goalRatioPerGameSample2[i] <- mean(R2$goalRatio[32:i])
}
for(i in 1:nrow(R2)){
        R2$goalRatioPerGameR2[i] <- summary(lm(goalRatioPerGameSample1 ~ goalRatioPerGameSample2, R2[1:i,]))$r.squared
}
R2$goalRatioPerGameR2[is.na(R2$goalRatioPerGameR2)] <- 0
R2$pointsPerGame <- 0
R2$pointsPerGameR2 <- 0
for(i in 1:nrow(R2)){
        R2$pointsPerGame[i] <- mean(R2$point[1:i])
}
for(i in 1:nrow(R2)){
        R2$pointsPerGameR2[i] <- summary(lm(pointsPerGame ~ goalRatioPerGameSample2, R2[1:i,]))$r.squared
}
...
for(i in 1:nrow(R2)){
        R2$shotRatioPerGameR2[i] <- summary(lm(shotRatioPerGame ~ goalRatioPerGameSample2, R2[1:i,]))$r.squared
}
...
for(i in 1:nrow(R2)){
        R2$shotValueRatioPerGameR2[i] <- summary(lm(shotValueRatioPerGame ~ goalRatioPerGameSample2, R2[1:i,]))$r.squared
}
R2$shotRatioPerGameR2[is.na(R2$shotRatioPerGameR2)] <- 0
...
g + geom_smooth(aes(colour = variable), method = "loess", se = F) + scale_y_continuous(limits  = c(0, 1)) +
        scale_x_continuous(breaks = seq(0, 32, 4)) + labs(y = "Correlation (R2)", title = "Predictors of Future Goal Ratios") + scale_colour_discrete("Metric", labels = c("Points per Game", "Goal Ratio", "Shot Ratio", "Shot Value Ratio")) +
        theme(legend.text = element_text(size = 8), plot.title = element_text(hjust = 0.5))
ggsave("Metrics Predictors of Future Goal Ratios.png", width = 10, height = 8)
Stats <- colMeans(R2)