...
library(ggplot2)
library(magrittr)
pitch_markings <- function(col_pitch = "dark green", col_lines = "white", x_scale = 1,
                           y_scale = 1, x_shift = 0, y_shift = 0, juego = FALSE) {
#Adds soccer pitch markings as a layer for use in a ggplot plot. 
#For instance: ggplot(DF, aes(x = x, y = y)) + pitch_markings() + geom_point() + theme_pitch()
#Assumes 100x100 coordinates.
#col_pitch: The colour of the pitch background. Default is dark green.
...
#Returns: ggplot layer
#Add pitch markings
markings <- list(geom_rect(xmin = 0*x_scale+x_shift, xmax = 100*x_scale+x_shift,
                           ymin = 0*y_scale+y_shift, ymax = 100*y_scale+y_shift,
                           colour  = col_lines, fill  = col_pitch, alpha = 1),
#Centre circle
                annotation_custom(grob=circleGrob(r=unit(1,"npc"), gp = gpar(col=col_lines, fill = col_pitch, lwd = 2)),
...
direction_label <- function(y_label = -3, x_label = 10) {
        require(ggplot2)
        require(grid)
        ...
        layer <- list(geom_segment(x = x_label + 2 , y = y_label, xend = x_label - 2, yend = y_label,
                                   arrow = arrow(length = unit(0.02, "npc")), colour = "gray75"),
                      annotate("text", x = x_label, y = y_label - 1, label = c("Direction of play"), 
                               vjust = 1, size = 3, colour = "gray75"))
        return(layer)
}
...
panel.background = element_blank(), panel.border = element_blank(), aspect.ratio = ifelse(is_landscape, 68/105, 105/68)) %>%
list() %>%
                return()
}